...
# FCNC
Standard modelの弱い相互作用は二種類に分けられる

- $W^{\pm}$ による charged current
- $Z$ による neurtal current

## Charged Current
Wボソンは左巻きフェルミオン、右巻き反フェルミオンとしか結合しない。この点では100%パリティ（右、左に対等）、もしくはCharge conjugation（粒子、反粒子に対等）破っている。
しかし未だCPは良い対称性を保っている。

## Neutral Current

<img src="./fig/eemumu.png"/>

ニュートリノは電荷を持たないので、$W$ボソン、$\gamma$ には結合しない。
$Z$ ボソンにのみ結合し、$Z$ ボソンも同様に左巻きのフェルミオンにしか結合しない。
実験的には左巻きのニュートリノしかなぜか存在していない（右巻きの反ニュートリノはある）。

$\mu \to e\gamma$ 、$Z\to e\mu$ は現在まで観測されていない。

### B中間子の崩壊

<img src="./fig/bmeson_decay.png"/>



## CKM行列
弱い相互作用はクォークに対して、質量固有状態に対して結合する。

$$
(d', s', b') = V(d,s,b)
$$

レプトンはニュートリノが質量を持っていない（SMの範疇では）ので、レプトン間の混合はしない。


## GIM機構
1970年に、Glahow, Iliopoulos, Maianiが「フレーバーを変えるが、電荷を変えない様な遷移は無い」と提唱した。
実験的結果からも今までにこの仮説は揺らいでいない。


$$
d_i'=\sum_{j}V_{ij}d_j
$$
ここで $d_1=d_L$、$d_2=s_L$、と表記する（Lは左巻きの意味）。

$$
\sum_{i}\bar{d_i}'d_i = \sum_{i,j,k}\bar{d_j}V_{ji}V_{ik}d_k = \sum_{i}\bar{d_i}d_i
$$

ここから $d\to d$、$s\to s$のフレーバーが変わらない遷移は許されているが、$s\to d$は禁止されている。
禁止されているツリーレベルのファインマン・ダイアグラムを次に示す。

<img src="./fig/fcnc.png"/>
